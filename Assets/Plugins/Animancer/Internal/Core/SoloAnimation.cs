...
namespace Animancer
{
    ...
    /// <remarks>
    ///     Documentation:
    ///     <see href="https://kybernetik.com.au/animancer/docs/manual/playing/component-types">Component Types</see>
    /// </remarks>
    /// <example>
    ...
    public class SoloAnimation : MonoBehaviour, IAnimationClipSource
    {
        ...
        [SerializeField]
        [Tooltip("Should the " + nameof(Clip) + " be automatically applied to the object in Edit Mode?")]
...
namespace Animancer.Editor
{
    ...
    [CustomEditor(typeof(SoloAnimation))]
    ...
    public class SoloAnimationEditor : UnityEditor.Editor
    {
        ...
        private void DoStopOnDisableGUI()
        {
                       ...
                       " Otherwise they will simply be paused and will resume from their current states when it is re-enabled."))
            {
                if (_KeepStateOnDisable != null)
                {
                    ...
                    var content = EditorGUI.BeginProperty(area, label, _KeepStateOnDisable);

                    _KeepStateOnDisable.boolValue = !EditorGUI.Toggle(area, content, !_KeepStateOnDisable.boolValue);
        ...
        private void DoRuntimeDetailsGUI()
        {
            ...
            else
            {
                ...
                var isPlaying = EditorGUILayout.Toggle("Is Playing", target.IsPlaying);
                if (EditorGUI.EndChangeCheck())
                    target.IsPlaying = isPlaying;

                EditorGUI.BeginChangeCheck();
                var time = EditorGUILayout.FloatField("Time", target.Time);
                if (EditorGUI.EndChangeCheck())
                    target.Time = time;
                ...
                EditorGUI.BeginChangeCheck();
                time = EditorGUILayout.Slider("Normalized Time", time, 0, 1);
                if (EditorGUI.EndChangeCheck())
