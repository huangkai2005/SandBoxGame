        ...
        ControllerState.ITransition, ICopyable<ControllerTransition>
    {
        ...
        public class Drawer : TransitionDrawer
        {
            ...
            public Drawer(params string[] parameters) : base(ControllerFieldName)
            {
                ...
                for (var i = 0; i < ParameterPropertySuffixes.Length; i++)
                    ...
                    if (EditorGUI.DropdownButton(area, label, FocusType.Passive))
                    {
                        ...
                        for (var i = 0; i < parameters.Length; i++)
                        {
                            ...
                            Serialization.AddPropertyModifierFunction(menu, property, parameter.name,
                                ...
                                targetProperty => { targetProperty.stringValue = parameter.name; });
                        }
            ...
            private static string GetFirstFloatParameterName(AnimatorController controller)
            {
                ...
                {
                    var parameter = parameters[i];
                    if (parameter.type == AnimatorControllerParameterType.Float) return parameter.name;
                }
